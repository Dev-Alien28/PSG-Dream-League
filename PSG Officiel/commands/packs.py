# commands/packs.py - VERSION FINALE
import discord
import random
import os
...
async def packs_command(interaction: discord.Interaction):
    ...
    if os.path.exists(image_path):
        ...
        # ✅ ephemeral=True : seul l'utilisateur voit la boutique
        await interaction.response.send_message(embed=embed, view=view, file=image_file, ephemeral=True)
    else:
...
async def buy_pack(interaction: discord.Interaction, pack_key: str):
    ...
    # ✅ Message de mention visible par tous AVANT l'embed
    mention_content = f"🎉 {interaction.user.mention} a obtenu une carte !"
    ...
    if image_file:
        ...
        # ✅ Envoyer dans le salon public avec mention (pas ephemeral)
        await interaction.response.send_message(content=mention_content, embed=embed, file=image_file)
    else:
        ...
        # ✅ Envoyer dans le salon public avec mention (pas ephemeral)
        await interaction.response.send_message(content=mention_content, embed=embed)
