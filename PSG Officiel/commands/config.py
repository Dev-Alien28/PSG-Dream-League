# commands/config.py - VERSION COMPLÈTE AVEC RAPPELS CONFIGURABLES
import discord
...
class ConfigMainView(View):
    ...
    @discord.ui.button(label="📢 Rappels Automatiques", style=discord.ButtonStyle.primary, row=1)
    async def config_reminders(self, interaction: discord.Interaction, button: Button):
        await interaction.response.edit_message(
            embed=self.create_reminders_embed(),
            view=ConfigRemindersView(self.interaction)
        )
    
    @discord.ui.button(label="🚫 Salons Sans Coins", style=discord.ButtonStyle.primary, row=2)
    async def config_no_coins(self, interaction: discord.Interaction, button: Button):
        await interaction.response.edit_message(
            embed=self.create_no_coins_embed(),
            view=ConfigNoCoinsView(self.interaction)
        )
    
    @discord.ui.button(label="📊 Voir Configuration Complète", style=discord.ButtonStyle.success, row=2)
    ...
    def create_reminders_embed(self):
        """Créer l'embed de configuration des rappels automatiques"""
        embed = discord.Embed(
            title="📢 Configuration des Rappels Automatiques",
            description="Configure les rappels automatiques personnalisables.",
            color=PSG_BLUE
        )
        
        bot = self.interaction.client
        if hasattr(bot, 'auto_reminder'):
            reminder = bot.auto_reminder
            channel_id = reminder.get_channel_id(self.guild_id)
            is_enabled = reminder.is_enabled(self.guild_id)
            interval = reminder.get_interval(self.guild_id)
            discussion_channel_id = reminder.get_discussion_channel_id(self.guild_id)
            
            if channel_id:
                channel = self.interaction.guild.get_channel(int(channel_id))
                if channel:
                    embed.add_field(
                        name="📢 Salon de rappels",
            ...
            status = "✅ **Activés**" if is_enabled else "❌ **Désactivés**"
            embed.add_field(
                name="📊 Statut",
                value=status,
                inline=True
            ...
            embed.add_field(
                name="⏰ Fréquence",
                value=f"Toutes les **{format_interval(interval)}**",
                inline=True
            )
            
            if discussion_channel_id:
                discussion_channel = self.interaction.guild.get_channel(int(discussion_channel_id))
            ...
            embed.add_field(
                name="⚠️ Erreur",
                value="Système de rappels non initialisé",
                inline=False
            )
    ...
    def create_no_coins_embed(self):
        """Créer l'embed de configuration des salons sans coins"""
        embed = discord.Embed(
            title="🚫 Configuration des Salons Sans Coins",
            description="Configure les salons où les membres ne gagnent **pas** de coins en parlant.\n"
                       "Utile pour les salons de commandes, règlement, annonces, etc.",
            color=PSG_BLUE
        )
        
        from utils.permissions import get_no_coins_channels
        no_coins_channels = get_no_coins_channels(self.guild_id)
        
        if no_coins_channels:
            channels_list = []
            for ch_id in no_coins_channels:
                channel = self.interaction.guild.get_channel(int(ch_id))
                if channel:
            ...
            if channels_list:
                embed.add_field(
                    name="Salons sans coins actuels",
                    value="\n".join(channels_list),
                    inline=False
                )
            else:
                embed.add_field(
                    name="Salons sans coins actuels",
                    value="Aucun ✅ (coins gagnés partout)",
                    inline=False
                )
        else:
            embed.add_field(
        ...
        embed.add_field(
            ...
            value="• Dans les salons **NON listés** : Les membres gagnent des coins\n"
                  "• Dans les salons **listés** : Aucun coin gagné",
            inline=False
        )
...
class ConfigRemindersView(View):
    """Vue pour configurer les rappels automatiques"""
    def __init__(self, interaction: discord.Interaction):
        super().__init__(timeout=300)
        self.interaction = interaction
        self.guild_id = str(interaction.guild_id)
        ...
        if len(channels) <= 25:
            ...
            self.reminder_select = Select(
                placeholder="Définir le salon de rappels",
                options=options,
                row=0
            )
            self.reminder_select.callback = self.set_reminder_channel
            self.add_item(self.reminder_select)
    
    async def set_reminder_channel(self, interaction: discord.Interaction):
        channel_id = self.reminder_select.values[0]
        channel = interaction.guild.get_channel(int(channel_id))
        ...
        if hasattr(bot, 'auto_reminder'):
            bot.auto_reminder.set_reminder_channel(self.guild_id, channel_id)
            
            embed = discord.Embed(
                title="✅ Salon de rappels configuré",
                description=f"{channel.mention} recevra les rappels automatiques.",
                color=PSG_BLUE
            )
            
            embed.add_field(
                name="⚠️ Important",
                value="Le salon est configuré mais les rappels sont **désactivés** par défaut.\n"
                      "Utilise le bouton '✅ Activer les rappels' pour les démarrer.",
                inline=False
            )
            
            await interaction.response.edit_message(embed=embed, view=ConfigRemindersView(self.interaction))
        else:
            await interaction.response.send_message("❌ Système de rappels non disponible!", ephemeral=True)
    
    @discord.ui.button(label="✅ Activer les rappels", style=discord.ButtonStyle.success, row=1)
    async def enable_reminders(self, interaction: discord.Interaction, button: Button):
        bot = interaction.client
        if hasattr(bot, 'auto_reminder'):
            success = bot.auto_reminder.enable_reminders(self.guild_id)
            
            if success:
                channel_id = bot.auto_reminder.get_channel_id(self.guild_id)
                channel = interaction.guild.get_channel(int(channel_id)) if channel_id else None
                interval = bot.auto_reminder.get_interval(self.guild_id)
                
                embed = discord.Embed(
                    title="✅ Rappels activés",
                    description=f"Les rappels automatiques sont maintenant actifs !\n\n"
                               f"📢 Salon : {channel.mention if channel else 'Non configuré'}\n"
                               f"⏰ Fréquence : Toutes les {format_interval(interval)}",
                    color=PSG_BLUE
                )
            else:
                embed = discord.Embed(
                    title="❌ Erreur",
                    description="Aucun salon de rappel n'est configuré.\n"
                               "Configure d'abord un salon avant d'activer les rappels.",
                    color=PSG_RED
                )
    ...
    @discord.ui.button(label="❌ Désactiver les rappels", style=discord.ButtonStyle.danger, row=1)
    async def disable_reminders(self, interaction: discord.Interaction, button: Button):
        bot = interaction.client
        if hasattr(bot, 'auto_reminder'):
            bot.auto_reminder.disable_reminders(self.guild_id)
            
            embed = discord.Embed(
                title="✅ Rappels désactivés",
                description="Les rappels automatiques sont maintenant désactivés.\n\n"
                           "Le salon reste configuré et tu peux les réactiver à tout moment.",
                color=PSG_BLUE
            )
    ...
    @discord.ui.button(label="⏰ Modifier le délai", style=discord.ButtonStyle.primary, row=2)
    async def change_interval(self, interaction: discord.Interaction, button: Button):
        await interaction.response.edit_message(
            embed=self.create_interval_embed(),
            view=ConfigReminderIntervalView(self.interaction)
        )
    
    @discord.ui.button(label="💬 Salon de discussion", style=discord.ButtonStyle.primary, row=2)
    async def set_discussion_channel(self, interaction: discord.Interaction, button: Button):
        await interaction.response.edit_message(
            embed=self.create_discussion_embed(),
            view=ConfigReminderDiscussionView(self.interaction)
        )
    
    @discord.ui.button(label="🗑️ Supprimer tout", style=discord.ButtonStyle.danger, row=3)
    async def remove_reminders(self, interaction: discord.Interaction, button: Button):
        bot = interaction.client
        if hasattr(bot, 'auto_reminder'):
            bot.auto_reminder.remove_reminder_channel(self.guild_id)
            
            embed = discord.Embed(
                title="✅ Configuration supprimée",
                description="Toute la configuration des rappels a été supprimée.",
                color=PSG_BLUE
            )
    ...
    def create_interval_embed(self):
        bot = self.interaction.client
        current_interval = 6.0
        
        if hasattr(bot, 'auto_reminder'):
            current_interval = bot.auto_reminder.get_interval(self.guild_id)
        
        embed = discord.Embed(
            title="⏰ Configuration du délai",
            description=f"Intervalle actuel : **{format_interval(current_interval)}**\n\n"
                       "Choisis un nouveau délai entre les rappels :",
            color=PSG_BLUE
        )
        
        return embed
    
    def create_discussion_embed(self):
        bot = self.interaction.client
        current_channel = None
        
        if hasattr(bot, 'auto_reminder'):
            channel_id = bot.auto_reminder.get_discussion_channel_id(self.guild_id)
            if channel_id:
                current_channel = self.interaction.guild.get_channel(int(channel_id))
        
        embed = discord.Embed(
            title="💬 Configuration du salon de discussion",
            description="Définis le salon à mentionner dans les rappels.\n\n"
                       f"Salon actuel : {current_channel.mention if current_channel else '**Non configuré**'}",
            color=PSG_BLUE
        )
...
class ConfigReminderIntervalView(View):
    """Vue pour choisir l'intervalle des rappels"""
    def __init__(self, interaction: discord.Interaction):
        super().__init__(timeout=300)
        self.interaction = interaction
        self.guild_id = str(interaction.guild_id)
        ...
        options = [
            discord.SelectOption(label="1 minute",   value="0.0167", emoji="⚡"),
            discord.SelectOption(label="5 minutes",  value="0.0833", emoji="⚡"),
            discord.SelectOption(label="10 minutes", value="0.1667", emoji="⚡"),
            discord.SelectOption(label="15 minutes", value="0.25",   emoji="⏱️"),
            discord.SelectOption(label="30 minutes", value="0.5",    emoji="⏱️"),
            discord.SelectOption(label="45 minutes", value="0.75",   emoji="⏱️"),
            discord.SelectOption(label="1 heure",    value="1",      emoji="⏰"),
            discord.SelectOption(label="2 heures",   value="2",      emoji="⏰"),
            discord.SelectOption(label="3 heures",   value="3",      emoji="⏰"),
            discord.SelectOption(label="4 heures",   value="4",      emoji="⏰"),
            discord.SelectOption(label="6 heures (recommandé)", value="6", emoji="✅"),
            discord.SelectOption(label="8 heures",   value="8",      emoji="⏰"),
            discord.SelectOption(label="12 heures",  value="12",     emoji="⏰"),
            discord.SelectOption(label="24 heures",  value="24",     emoji="⏰"),
        ]
        
        self.interval_select = Select(
            placeholder="Choisis un intervalle",
            options=options,
            row=0
        )
        self.interval_select.callback = self.set_interval
        self.add_item(self.interval_select)
    
    async def set_interval(self, interaction: discord.Interaction):
        hours = float(self.interval_select.values[0])
        label = format_interval(hours)
        
        bot = interaction.client
        if hasattr(bot, 'auto_reminder'):
            bot.auto_reminder.set_interval(self.guild_id, hours)
            
            embed = discord.Embed(
                title="✅ Intervalle modifié",
                description=f"Les rappels seront maintenant envoyés toutes les **{label}**.",
                color=PSG_BLUE
            )
            
            embed.add_field(
                name="ℹ️ Information",
                value=f"Le prochain rappel sera envoyé dans maximum {label}.",
                inline=False
            )
    ...
    async def back_to_reminders(self, interaction: discord.Interaction, button: Button):
        main_view = ConfigMainView(self.interaction)
        await interaction.response.edit_message(
            embed=main_view.create_reminders_embed(),
            view=ConfigRemindersView(self.interaction)
        )


class ConfigReminderDiscussionView(View):
    """Vue pour choisir le salon de discussion"""
    def __init__(self, interaction: discord.Interaction):
        super().__init__(timeout=300)
        self.interaction = interaction
        self.guild_id = str(interaction.guild_id)
        ...
        if len(channels) <= 25:
            ...
            self.discussion_select = Select(
                placeholder="Choisis le salon de discussion",
                options=options,
                row=0
            )
            self.discussion_select.callback = self.set_discussion
            self.add_item(self.discussion_select)
    
    async def set_discussion(self, interaction: discord.Interaction):
        channel_id = self.discussion_select.values[0]
        channel = interaction.guild.get_channel(int(channel_id))
        ...
        bot = interaction.client
        if hasattr(bot, 'auto_reminder'):
            bot.auto_reminder.set_discussion_channel(self.guild_id, channel_id)
            
            embed = discord.Embed(
                title="✅ Salon de discussion configuré",
                description=f"Les rappels mentionneront maintenant {channel.mention} pour discuter.",
                color=PSG_BLUE
            )
            
            embed.add_field(
                name="📝 Message qui sera affiché",
                value=f"💬 Pour discuter, utilisez le salon {channel.mention}",
                inline=False
            )
    ...
    @discord.ui.button(label="🗑️ Retirer le salon", style=discord.ButtonStyle.danger, row=1)
    async def remove_discussion(self, interaction: discord.Interaction, button: Button):
        bot = interaction.client
        if hasattr(bot, 'auto_reminder'):
            bot.auto_reminder.set_discussion_channel(self.guild_id, None)
            
            embed = discord.Embed(
                title="✅ Salon de discussion retiré",
                description="Le rappel affichera le message par défaut.",
                color=PSG_BLUE
            )
            
            embed.add_field(
                name="📝 Message par défaut",
                value="💬 Pour discuter, utilisez les autres salons du serveur !",
                inline=False
            )
...
class ConfigNoCoinsView(View):
    """Vue pour configurer les salons sans coins"""
    def __init__(self, interaction: discord.Interaction):
        super().__init__(timeout=300)
        self.interaction = interaction
        self.guild_id = str(interaction.guild_id)
        ...
        if len(channels) <= 25:
            ...
            self.add_channel_select = Select(
                placeholder="Ajouter un salon sans coins",
                options=add_options,
                row=0
            )
            self.add_channel_select.callback = self.add_no_coins_channel
            self.add_item(self.add_channel_select)
        
        from utils.permissions import get_no_coins_channels
        no_coins = get_no_coins_channels(self.guild_id)
        if no_coins:
            remove_options = []
            for ch_id in no_coins[:25]:
                channel = interaction.guild.get_channel(int(ch_id))
                if channel:
                    remove_options.append(
                        discord.SelectOption(
            ...
            if remove_options:
                self.remove_channel_select = Select(
                    placeholder="Retirer un salon sans coins",
                    options=remove_options,
                    row=1
                )
                self.remove_channel_select.callback = self.remove_no_coins_channel
                self.add_item(self.remove_channel_select)
    
    async def add_no_coins_channel(self, interaction: discord.Interaction):
        channel_id = self.add_channel_select.values[0]
        channel = interaction.guild.get_channel(int(channel_id))
        ...
        from utils.permissions import get_no_coins_channels, add_no_coins_channel
        no_coins = get_no_coins_channels(self.guild_id)
        if channel_id in no_coins:
            await interaction.response.send_message(
                f"⚠️ {channel.mention} est déjà dans la liste des salons sans coins",
                ephemeral=True
            )
            return
        
        add_no_coins_channel(self.guild_id, channel_id)
        
        embed = discord.Embed(
            title="✅ Salon ajouté",
            description=f"{channel.mention} est maintenant un salon **sans coins**.\n\n"
                       f"Les membres ne gagneront plus de coins en parlant dans ce salon.",
            color=PSG_BLUE
        )
        
        all_no_coins = get_no_coins_channels(self.guild_id)
        channels_list = []
        for ch_id in all_no_coins:
            ch = interaction.guild.get_channel(int(ch_id))
            if ch:
                channels_list.append(f"• {ch.mention}")
        ...
        if channels_list:
            embed.add_field(
                name="Tous les salons sans coins",
                value="\n".join(channels_list),
                inline=False
            )
    ...
    async def remove_no_coins_channel(self, interaction: discord.Interaction):
        channel_id = self.remove_channel_select.values[0]
        channel = interaction.guild.get_channel(int(channel_id))
        
        from utils.permissions import remove_no_coins_channel
        success = remove_no_coins_channel(self.guild_id, channel_id)
        ...
        embed = discord.Embed(
            title="✅ Salon retiré",
            description=f"{channel.mention if channel else 'Le salon'} n'est plus dans la liste sans coins.\n\n"
                       f"Les membres peuvent maintenant gagner des coins en parlant dans ce salon.",
            color=PSG_BLUE
        )
        ...
        if all_no_coins:
            ...
            if channels_list:
                embed.add_field(
                    name="Salons sans coins restants",
                    value="\n".join(channels_list),
                    inline=False
                )
        else:
            embed.add_field(
                name="ℹ️ Info",
                value="Plus aucun salon sans coins. Les membres peuvent gagner des coins partout.",
                inline=False
            )
...
def create_main_embed(interaction: discord.Interaction):
    ...
    embed.add_field(
        ...
        value="Configure les rappels personnalisables (délai, salon de discussion)",
        inline=False
    )
    
    embed.add_field(
        name="🚫 Salons Sans Coins",
        value="Définis les salons où les membres ne gagnent pas de coins en parlant",
        inline=False
    )
...
async def create_full_config_embed(interaction: discord.Interaction):
    ...
    if hasattr(bot, 'auto_reminder'):
        reminder = bot.auto_reminder
        channel_id = reminder.get_channel_id(guild_id)
        is_enabled = reminder.is_enabled(guild_id)
        interval = reminder.get_interval(guild_id)
        discussion_channel_id = reminder.get_discussion_channel_id(guild_id)
        ...
        if channel_id:
            ...
            disc_ch = None
            if discussion_channel_id:
                disc_ch = interaction.guild.get_channel(int(discussion_channel_id))
            
            value = f"{channel.mention if channel else 'Salon inconnu'}\n{status} • {format_interval(interval)}"
            if disc_ch:
                value += f"\n💬 {disc_ch.mention}"
        else:
            value = "Non configuré ❌"
    ...
    if no_coins_channels:
        ...
        embed.add_field(
            name="🚫 Salons Sans Coins",
            value="\n".join(channels_list) if channels_list else "Aucun",
            inline=False
        )
    else:
        embed.add_field(
